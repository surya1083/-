...
export default function App() {
  ...
  const handleInput = (value) => {
    ...
    if (!value.trim()) {
      setArray([]);
      setDisplayArray([]);
      setStep('Enter an array to start!');
      return;
    }
    ...
    if (nums.length > 0 && nums.length <= 15 && invalidParts.length === 0) {
      ...
      setStep(`Input array set: [${nums.join(',')}]`);
    } else {
      setArray([]);
      setDisplayArray([]);
      setStep(`Invalid input! Use 1-15 comma-separated integers (e.g., -1,1000,23). ${invalidParts.length > 0 ? `Invalid: ${invalidParts.join(', ')}` : 'Check for extra commas or spaces.'}`);
    }
  };

  const runAlgorithm = async () => {
    if (array.length === 0) {
      setStep('Enter a valid array first!');
      return;
    }
    setIsRunning(true);
    setIsPaused(false);
    ...
    await selectedAlgo.visualize([...array], setDisplayArray, setStep, speed, pausedRef, cancelRef);
    setIsRunning(false);
    setIsPaused(false);
    pausedRef.current = false;
    cancelRef.current = false;
  ...
  const togglePause = () => {
    ...
    setStep(!isPaused ? `Paused: ${selectedAlgo.name}` : `Resumed: ${selectedAlgo.name}`);
  };
  ...
  return (
    <div className="app">
      <header>
        <h1>AlgorithmSortingVisualizer</h1>
